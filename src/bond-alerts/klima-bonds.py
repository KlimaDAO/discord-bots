...
def check_is_worth(stakingRewards, rebase, bondDiscount):
    if stakingRewards >= bondDiscount + decimal.Decimal(((1+rebase))*(1-(1+rebase)**15)/(1-(1+rebase)))/15 - 1:
        ...
        return(':white_check_mark:', bondDiscount + decimal.Decimal(((1+rebase))*(1-(1+rebase)**15)/(1-(1+rebase)))/15 - 1)  # noqa: E501
...
def get_prices():
    ...
    if datetime.datetime.now() - datetime.timedelta(seconds=90) >= last_call:
        ...
        klimaPriceUSD = lp_base_token_price(lp_address='0x9803c7ae526049210a1725f7487af26fe2c24614', basePrice=bctPriceUSD)  # noqa: E501
    ...
    if len(urows) > 0:
        for (user_id,) in urows:
            ...
            for (bond, discount, is_active) in rows:
                if bond == 'BCT':
                    if is_active == 0 and bctClosed is False and discount <= 100 * bctDisc:
                        user_discount = True
                        is_worth, maxBondDisc = check_is_worth(stakingRewards, rebase, bctDisc)
                        fields = fields + [
                            ('Bond Type:', f' {bond}', True), ('Discount:', f' {100*bctDisc:,.2f}%', True), ('\u200b', '\u200b', True),  # noqa: E501
                            ('Could be profitable?:', is_worth, True), ('Compound Discount:', f' {100*maxBondDisc:,.2f}%', True), ("Max payout USD:", f'${bctMax:,.2f}', True)  # noqa: E501
                        ]
                        db.execute("UPDATE bond_alerts SET Active = 1 WHERE UserID = ? AND Bond = ? AND Discount = ?", user_id, bond, discount)  # noqa: E501
                        db.save()
                        print(f'Alert sent to {user_id} for {bond} at more than {discount}% discount')
                        print(fields)
                    elif is_active == 1 and discount > 100 * bctDisc:
                        db.execute("UPDATE bond_alerts SET Active = 0 WHERE UserID = ? AND Bond = ? AND Discount = ?", user_id, bond, discount)  # noqa: E501
                        db.save()
                        print(f'Alert reseted for {bond} at more than {discount}% discount')

                if bond == 'KLIMA-BCT':
                    if is_active == 0 and klimaBctClosed is False and discount <= 100 * klimaBctDisc:
                        user_discount = True
                        is_worth, maxBondDisc = check_is_worth(stakingRewards, rebase, klimaBctDisc)
                        fields = fields + [
                            ('Bond Type:', f' {bond}', True), ('Discount:', f' {100*klimaBctDisc:,.2f}%', True), ('\u200b', '\u200b', True),  # noqa: E501
                    ...
                    if is_active == 0 and bctUsdClosed is False and discount <= 100 * bctUsdDisc:
                        user_discount = True
                        is_worth, maxBondDisc = check_is_worth(stakingRewards, rebase, bctUsdDisc)
                        fields = fields + [
                            ('Bond Type:', f' {bond}', True), ('Discount:', f' {100*bctUsdDisc:,.2f}%', True), ('\u200b', '\u200b', True),  # noqa: E501
...
async def bonds(ctx):
    ...
    info = sorted(
        [
            ...
            ('KLIMA-BCT', check_is_worth(stakingRewards, rebase, klimaBctDisc), 100*klimaBctDisc, klimaBctMax, klimaBctClosed),  # noqa: E501
            ('BCT-USDC', check_is_worth(stakingRewards, rebase, bctUsdDisc), 100*bctUsdDisc, bctUsdMax, bctUsdClosed)  # noqa: E501
    ...
    for lf in lfields:
        page = page + 1
        embed = discord.Embed(title="Current KlimaDAO Bond Discounts", descriotion=f":small_blue_diamond: 5 day ROI for **staking** sits at **{100 * stakingRewards:,.2f}%** \n :small_blue_diamond: Current **KLIMA** price is **${klimaPriceUSD:,.2f}**", colour=0xFFFFFF)  # noqa: E501
        for f in lf:
            if len(f) > 2:
...
async def info_bonds(ctx):
    ...
    bonds = [('BCT', '[Link](https://polygonscan.com/address/0x7De627C56D26529145a5f9D85948ecBeAF9a4b34)'), ('KLIMA-BCT', '[Link](https://polygonscan.com/address/0x1E0Dd93C81aC7Af2974cdB326c85B87Dd879389B)'), ('BCT-USDC', '[Link](https://polygonscan.com/address/0xBF2A35efcd85e790f02458Db4A3e2f29818521c5)')]  # noqa: E501
    fields = []
    b = 0
    for b in bonds:
        fields += [('Bond Type:', b[0], True), ('\u200b', '\u200b', True), ('Contract:', b[1], True)]

    embed = discord.Embed(title="KlimaDAO Bond Types", descriotion=f"KlimaDAO currently offers {len(bonds)} bond types.", colour=0xFFFFFF)  # noqa: E501
    for name, value, inline in fields:
        embed.add_field(name=name, value=value, inline=inline)
...
async def create_alert(
    ...
    if bond not in bond_list:
        embed = discord.Embed(title="Configuration Error", descriotion=f"Incorrect bond type: **{bond}**. \n  The input value for a bond must be within the following: **{bond_str}** ", colour=0xd14d4d)  # noqa: E501
        await ctx.respond(embed=embed)
    else:
        alert_id = db.record("SELECT AlertID FROM bond_alerts WHERE UserID = ? AND Bond = ? AND Discount=?", ctx.author.id, bond, min_discount)  # noqa: E501
        if alert_id is None:
            if len(db.records("SELECT * FROM bond_alerts WHERE UserID = ?", ctx.author.id)) == 5:
                embed = discord.Embed(title="Configuration Error", descriotion=f"{ctx.author.mention} already had 5 alerts configured. Don't be greedy ser!", colour=0xd14d4d)  # noqa: E501
                await ctx.respond(embed=embed)
            else:
                db.execute("INSERT INTO bond_alerts VALUES (?,?,?,?,0)", str(ctx.author.id) + bond + str(min_discount), ctx.author.id, bond, min_discount)  # noqa: E501
                db.save()
                embed = discord.Embed(title="New Bond Alert Configured", descriotion=f'{ctx.author.mention} has created a new alert for discounts over {min_discount}% on {bond} bonds!', colour=0x17d988)  # noqa: E501
                await ctx.respond(embed=embed)
        else:
            embed = discord.Embed(title="Bond Alert Already Configured", descriotion=f'{ctx.author.mention} already had an alert for discounts over {min_discount}% on {bond} bonds.', colour=0xd14d4d)  # noqa: E501
            await ctx.respond(embed=embed)
...
async def delete_alert(
    ...
    if alert_id is not None:
        ...
        db.save()
        embed = discord.Embed(title="Bond Alert Deleted", descriotion=f'{ctx.author.mention} has deleted an alert for discounts over {min_discount}% on {bond} bonds!', colour=0x17d988)  # noqa: E501
        await ctx.respond(embed=embed)
    else:
        embed = discord.Embed(title="Alert Doesn't Exist", descriotion=f'{ctx.author.mention} does not have any alerts for discounts over {min_discount}% on {bond} bonds.', colour=0xFFFFFF)  # noqa: E501
        await ctx.respond(embed=embed)
...
async def delete_all(ctx):
    ...
    if alert_id is not None:
        ...
        db.save()
        embed = discord.Embed(title="All Bond Alerts Deleted", descriotion=f'{ctx.author.mention} has deleted all bond alerts!', colour=0x17d988)  # noqa: E501
        await ctx.respond(embed=embed)
    else:
        embed = discord.Embed(title="No Alerts Configured", descriotion=f'{ctx.author.mention} does not have any configured alerts.', colour=0xFFFFFF)  # noqa: E501
        await ctx.respond(embed=embed)
...
async def my_alerts(ctx):
    ...
    if alert_id is None:
        ...
        for (bond, discount) in rows:
            embed.add_field(name='Bond Type:', value=f'{bond}', inline=True)
...
async def help_bonds(ctx):
    ...
    embed.add_field(name=':small_blue_diamond: /bonds', value='Returns all the live information for every bond type issues by KlimaDAO.', inline=False)  # noqa: E501
    embed.add_field(name=':small_blue_diamond: /my_alerts', value='Returns a list with all the alerts that the user has.', inline=False)  # noqa: E501
